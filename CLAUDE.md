...
4. Sidebar inputs produce `baseline_elapsed` + a `reset_key`; `cursor_from_elapsed()` turns that into a **play cursor**
...
- `_cursor_key` — `(game_id, mode, reset_key, safety_margin)`; a change re-seeds both from the mode's baseline
...
Each mode's only job is to produce `baseline_elapsed` plus a `reset_key`. The cursor logic then runs once, after the branch.

- *Started at* — elapsed wall-clock minutes since a chosen datetime, via `elapsed_game_seconds()` (190 broadcast min → 3600 game seconds linear mapping). **Ratchets** rather than re-seeding: its baseline recomputes every rerun, so real time only ever raises `_cursor_max`, and `_cursor_idx` rides that leading edge unless the user has scrubbed back.
- *X minutes in* — direct broadcast-minute input
- *Jump to game clock* — quarter + MM:SS input, converts to game-seconds elapsed
- *Start at kickoff* — baseline 0; the replay is driven entirely from the time bar
...
**Auto-advance:** When `auto=True`, `st_autorefresh` fires and the block at the bottom of the file bumps `_cursor_max` by one. *Started at* is excluded — its advancement already comes from the ratchet, and stepping as well would outrun the broadcast it tracks.
